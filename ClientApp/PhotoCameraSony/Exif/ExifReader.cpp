...
#include "exif.h"
/*
	FILE *fp = fopen("C:/sample.jpg", "rb");
	if (!fp)
	{
		qDebug() << "cannot open image";
	}
	fseek(fp, 0, SEEK_END);
	...
	qDebug() << "Original date/time   : " << result.DateTimeOriginal.c_str();
...
QMap<ExifReader::ExifParam, QString> ExifReader::getMetadata(QByteArray data)
{
	

	// Parse EXIF
	easyexif::EXIFInfo result;
	int code = result.parseFrom(reinterpret_cast<unsigned char*>(data.data()), data.count());
	if (code)
	{
		qWarning() << "Error parsing EXIF: code %d\n", code;
		return {};
	}

	return { { ExifParam::OriginalDateTime, QString::fromStdString(result.DateTimeOriginal) } };
}
